...
class TrelloImporter:
    ...
    def list_projects(self):
        projects_data = self._client.get(
            "/members/me/boards",
            {
    ...
    def list_users(self, project_id):
        members = []
        for member in self._client.get(
            "/board/{}/members/all".format(project_id), {"fields": "id"}
        ):
    ...
    def _import_project_data(self, data, options):
        ...
        (
            can_create,
            error_message,
        ) = projects_service.check_if_project_can_be_created_or_updated(project)
        if not can_create:
            raise exceptions.NotEnoughSlotsForProject(
                project.is_private, 1, error_message
            )
        project.save()
        ...
        UserStoryCustomAttribute.objects.create(
            name="Due", description="Due date", type="date", order=1, project=project
        )
        import_service.create_memberships(
            options.get("users_bindings", {}), project, self._user, "trello"
        )
    ...
    def _import_user_stories_data(self, data, project, options):
        ...
        for card in cards:
            if card["closed"] and not options.get("import_closed_data", False):
                continue
            if statuses[card["idList"]]["closed"] and not options.get(
                "import_closed_data", False
            ):
                continue
    ...
    def _import_attachments(self, us, card, options):
        ...
        for attachment in card["attachments"]:
            ...
            att.attached_file.save(
                attachment["name"], ContentFile(data.content), save=True
            )

            UserStory.objects.filter(
                id=us.id, created_date__gt=attachment["date"]
            ).update(created_date=attachment["date"])
    ...
    def _transform_action_data(self, us, action, statuses, options):
        ...
        user = {
            "pk": None,
            "name": action.get("memberCreator", {}).get("fullName", None),
        }
