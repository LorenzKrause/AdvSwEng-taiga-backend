...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument("--token", dest="token", type=str, help="Auth token")
        parser.add_argument(
            "--project-id",
            dest="project_id",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if options.get("token", None):
            ...
            print(
                "Go to here and come with your code (in the redirected url): {}".format(
                    url
                )
            )
            code = input("Code: ")
        ...
        for user in importer.list_users(project_id):
            while True:
                if user["detected_user"] is not None:
                    print(
                        "User automatically detected: {} as {}".format(
                            user["full_name"], user["detected_user"]
                        )
                    )
                ...
                username_or_email = input(
                    "{}: ".format(user["full_name"] or user["username"])
                )
                if username_or_email == "":
                    break
                try:
                    users_bindings[user["id"]] = User.objects.get(
