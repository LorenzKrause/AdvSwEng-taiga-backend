...
class GithubImporter:
    ...
    def list_users(self, project_full_name):
        collaborators = self._client.get(
            "/repos/{}/collaborators".format(project_full_name)
        )
        collaborators = [
            self._client.get("/users/{}".format(u["login"])) for u in collaborators
        ]
        return [
            {
                "id": u["id"],
                ...
                "detected_user": self._get_user(u),
            }
    ...
    def _import_project_data(self, repo, options):
        ...
        import_service.create_memberships(
            options.get("users_bindings", {}), project, self._user, "github"
        )

        for milestone in self._client.get(
            "/repos/{}/milestones".format(repo["full_name"])
        ):
            taiga_milestone = Milestone.objects.create(
                ...
                if milestone["due_on"]
                else datetime.date(datetime.MAXYEAR, 12, 31),
            )
    ...
    def _import_user_stories_data(self, project, repo, options):
        ...
        while True:
            ...
            for issue in issues:
                ...
                assigned_to = users_bindings.get(
                    issue["assignee"]["id"] if issue["assignee"] else None, None
                )
            ...
            for comment in comments:
                ...
                snapshot = take_snapshot(
                    ...
                    user=users_bindings.get(
                        comment["user"]["id"],
                        User(
                            full_name=comment["user"].get("name", None)
    ...
    def _import_history(self, project, repo, options):
        ...
        while True:
            events = self._client.get(
                "/repos/{}/issues/events".format(repo["full_name"]),
                {"page": page, "per_page": 100},
            )
            page += 1
        ...
        for event in sorted(all_events, key=lambda x: x["id"]):
            if options.get("type", None) == "user_stories":
                obj = UserStory.objects.get(
                    project=project, ref=event["issue"]["number"]
                )
            elif options.get("type", None) == "issues":
    ...
    def _transform_event_data(self, obj, event, options, cumulative_data):
        ...
        taiga_user = (
            users_bindings.get(event["actor"]["id"], None) if event["actor"] else None
        )
        if taiga_user:
        ...
        if event["event"] == "renamed":
            ...
            if isinstance(obj, Issue):
                result["change_old"]["status"] = obj.project.issue_statuses.get(
                    name="Closed"
                ).id
                result["change_new"]["status"] = obj.project.issue_statuses.get(
                    name="Open"
                ).id
            ...
            UnassignedEventHandler(result, cumulative_data, users_bindings).handle(
                event
            )
        elif event["event"] == "demilestoned":
            if isinstance(obj, UserStory):
                try:
                    result["change_old"]["milestone"] = obj.project.milestones.get(
                        name=event["milestone"]["title"]
                    ).id
                except Milestone.DoesNotExist:
                    result["change_old"]["milestone"] = 0
                    result["update_values"] = {
                        "milestone": {"0": event["milestone"]["title"]}
                    }
            ...
            if isinstance(obj, UserStory):
                ...
                if cumulative_data["milestone"] is not None:
                    result["update_values"]["milestone"][
                        str(cumulative_data["milestone"])
                    ] = cumulative_data["milestone_name"]
                result["change_old"]["milestone"] = cumulative_data["milestone"]
                try:
                    taiga_milestone = obj.project.milestones.get(
                        name=event["milestone"]["title"]
                    )
    ...
    def get_auth_url(cls, client_id, callback_uri=None):
        if callback_uri is None:
            return "https://github.com/login/oauth/authorize?client_id={}&scope=user,repo".format(
                client_id
            )
        return "https://github.com/login/oauth/authorize?client_id={}&scope=user,repo&redirect_uri={}".format(
...
class AssignedEventHandler:
    ...
    def generate_update_values_from_cumulative_data(self, event):
        if self.cumulative_data["assigned_to_name"] is not None:
            self.result["update_values"]["users"][
                str(self.cumulative_data["assigned_to"])
            ] = self.cumulative_data["assigned_to_name"]
