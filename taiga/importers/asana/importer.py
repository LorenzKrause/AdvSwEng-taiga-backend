...
class AsanaImporter:
    ...
    def list_users(self, project_id):
        ...
        for ws in self._client.workspaces.find_all():
            for user in self._client.users.find_by_workspace(
                ws["id"], fields=["id", "name", "email", "photo"]
            ):
                users.append(
                    {
                        "id": user["id"],
    ...
    def _import_project_data(self, project, options):
        ...
        import_service.create_memberships(
            users_bindings, taiga_project, self._user, "asana"
        )
    ...
    def _import_user_stories_data(self, taiga_project, project, options):
        ...
        for task in tasks:
            ...
            us = UserStory.objects.create(
                ...
                status=taiga_project.us_statuses.get(
                    slug="closed" if task["completed"] else "open"
                ),
            ...
            UserStory.objects.filter(id=us.id).update(
                modified_date=task["modified_at"], created_date=task["created_at"]
            )
    ...
    def _import_task_data(self, taiga_project, us, assana_project, task, options):
        ...
        assigned_to = (
            users_bindings.get(task.get("assignee", {}).get("id", None)) or None
        )

        external_reference = None
        if options.get("keep_external_reference", False):
            external_url = "https://app.asana.com/0/{}/{}".format(
                assana_project["id"], task["id"],
            )
            external_reference = ["asana", external_url]
        ...
        taiga_task = Task.objects.create(
            ...
            owner=self._user,
            assigned_to=assigned_to,
            status=taiga_project.task_statuses.get(
                slug="closed" if task["completed"] else "open"
            ),
            us_order=task["id"],
        ...
        if task["due_on"]:
            taiga_task.custom_attributes_values.attributes_values = {
                due_date_field.id: task["due_on"]
            }
            ...
            if story["type"] == "comment":
                snapshot = take_snapshot(
                    ...
                    user=users_bindings.get(
                        story["created_by"]["id"],
                        User(full_name=story["created_by"]["name"]),
                    ),
                    delete=False,
                )
    ...
    def _import_attachments(self, obj, task, options):
        attachments = self._client.attachments.find_by_task(
            task["id"], fields=["name", "download_url", "created_at"]
        )
