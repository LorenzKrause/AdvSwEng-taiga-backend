...
class PivotalClient:
    ...
    def get_attachment(self, attachment_id):
        headers = {"X-TrackerToken": self.token}
        url = "https://www.pivotaltracker.com/file_attachments/{}/download".format(
            attachment_id
        )
...
class PivotalImporter:
    ...
    def _import_project_data(self, project_id, options):
        project_data = self._client.get(
            ...
            {
                "fields": ",".join(
                    ["point_scale", "name", "description", "labels(name)",]
                )
            },
        )
        project_data["iterations"] = self._client.get(
            "/projects/{}/iterations".format(project_id),
            {"fields": ",".join(["number", "start", "finish", "stories",])},
        )
        ...
        project_template.points = [{"value": None, "name": "?", "order": 1,}]
        ...
        UserStoryCustomAttribute.objects.create(
            name="Type", description="Story type", type="text", order=2, project=project
        )
        ...
        for iteration in project_data["iterations"]:
            ...
            Milestone.objects.filter(id=milestone.id).update(
                created_date=iteration["start"], modified_date=iteration["start"],
            )
        ...
        for iteration in project_data["iterations"]:
            for story in iteration["stories"]:
                story_milestone_binding[story["id"]] = Milestone.objects.get(
                    project=project, slug="sprint-{}".format(iteration["number"])
                )

        counter = 0
        offset = 0
        while True:
            stories = self._client.get(
                "/projects/{}/stories".format(project_data["id"]),
                {
                    "envelope": "true",
                    "limit": 300,
                    "offset": offset,
            ...
            for story in stories["data"]:
                ...
                points = Points.objects.get(
                    project=project, value=story.get("estimate", None)
                )
                RolePoints.objects.filter(user_story=us, role__slug="main").update(
                    points_id=points.id
                )
                ...
                if story.get("deadline", None):
                    us.custom_attributes_values.attributes_values = {
                        due_date_field.id: story["deadline"]
                    }
                    us.custom_attributes_values.save()
                if story.get("story_type", None):
                    us.custom_attributes_values.attributes_values = {
                        story_type_field.id: story["story_type"]
                    }
                    us.custom_attributes_values.save()
                    ...
                    if epics.get(label["id"], None):
                        RelatedUserStory.objects.create(
                            epic=Epic.objects.get(
                                project=project, ref=epics.get(label["id"])["id"]
                            ),
                            user_story=us,
    ...
    def _import_tasks(self, project_data, us, story):
        for task in story["tasks"]:
            taiga_task = Task.objects.create(
                ...
                status=us.project.task_statuses.get(
                    slug="complete" if task["complete"] else "incomplete"
                ),
                project=us.project,
    ...
    def _import_attachment(
        self, obj, attachment_id, attachment_name, created_at, person_id, options
    ):
        users_bindings = options.get("users_bindings", {})
    ...
    def _transform_activity_data(self, obj, activity, options):
        ...
        taiga_user = users_bindings.get(
            activity.get("performed_by", {}).get("id", None), None
        )
        if taiga_user:
        ...
        if activity["kind"] == "story_create_activity":
            UserStory.objects.filter(
                id=obj.id, created_date__gt=activity["occurred_at"]
            ).update(
            ...
            for change in activity["changes"]:
                if change["change_type"] != "update" or change["kind"] not in [
                    "story",
                    "epic",
                ]:
                    continue
                ...
                if "description" in change["new_values"]:
                    result["change_old"]["description"] = str(
                        change["original_values"]["description"]
                    )
                    result["change_new"]["description"] = str(
                        change["new_values"]["description"]
                    )
                    result["change_old"]["description_html"] = mdrender(
                        obj.project, str(change["original_values"]["description"])
                    )
                    result["change_new"]["description_html"] = mdrender(
                        obj.project, str(change["new_values"]["description"])
                    )
                ...
                if "estimate" in change["new_values"]:
                    ...
                    result["change_old"]["points"] = {
                        obj.project.roles.get(slug="main").id: old_points
                    }
                    result["change_new"]["points"] = {
                        obj.project.roles.get(slug="main").id: new_points
                    }
                ...
                if "labels" in change["new_values"]:
                    result["change_old"]["tags"] = [
                        l.lower() for l in change["original_values"]["labels"]
                    ]
                    result["change_new"]["tags"] = [
                        l.lower() for l in change["new_values"]["labels"]
                    ]

                if "current_state" in change["new_values"]:
                    result["change_old"]["status"] = obj.project.us_statuses.get(
                        slug=change["original_values"]["current_state"]
                    ).id
                    result["change_new"]["status"] = obj.project.us_statuses.get(
                        slug=change["new_values"]["current_state"]
                    ).id
            ...
            for change in activity["changes"]:
                ...
                try:
                    ...
                    if "description" in change["new_values"]:
                        result["change_old"]["subject"] = change["original_values"][
                            "description"
                        ]
                        result["change_new"]["subject"] = change["new_values"][
                            "description"
                        ]
                        result["obj"] = task
                    if "complete" in change["new_values"]:
                        result["change_old"]["status"] = obj.project.task_statuses.get(
                            slug="complete"
                            if change["original_values"]["complete"]
                            else "incomplete"
                        ).id
                        result["change_new"]["status"] = obj.project.task_statuses.get(
                            slug="complete"
                            if change["new_values"]["complete"]
                            else "incomplete"
                        ).id
                        result["obj"] = task
