...
class JiraNormalImporter(JiraImporterCommon):
        ...
        return [
            {
                ...
                "is_private": True,
                "importer_type": "normal",
            }
            for project in self._client.get("/project", {"expand": "description"})
        ]
    ...
    def _import_project_data(self, project_id, options):
        ...
        for us_status in us_statuses.values():
            ...
            counter += 1
        if us_statuses:
            project_template.default_options["us_status"] = list(us_statuses.values())[
                0
            ]["name"]
        ...
        for issue_type in types:
            ...
            while True:
                issues = self._client.get(
                    "/search",
                    {
                        "jql": "project={} AND issuetype={}".format(
                            project_id, issue_type["id"]
                        ),
                        "startAt": offset,
                        "fields": "*all",
                ...
                for issue in issues["issues"]:
                    ...
                    if (
                        options.get("keep_external_reference", False)
                        and "url" in issue["fields"]
                    ):
                    ...
                    if points_value:
                        ...
                        RolePoints.objects.filter(
                            user_story=us, role__slug="main"
            ...
            while True:
                ...
                for issue in issues["issues"]:
                    ...
                    taiga_issue = Issue.objects.create(
                        ...
                        owner=owner,
                        assigned_to=assigned_to,
                        status=project.issue_statuses.get(
                            name=issue["fields"]["status"]["name"]
                        ),
                        subject=issue["fields"]["summary"],
                        description=issue["fields"]["description"] or "",
            ...
            while True:
                ...
                for issue in issues["issues"]:
                    ...
                    if epic_key:
                        ...
                        us = project.user_stories.get(
                            ref=int(issue["key"].split("-")[1])
                        )
                        RelatedUserStory.objects.create(
                            user_story=us, epic=epic, order=1
                        )
