    ...
    for link in links:
        ...
        if importing_project_key == project_key:
            ...
            url = importer._client.server + "/projects/{}/issues/{}-{}".format(
                project_key, project_key, issue_key
            )
            richtext += "  * This item {} [{}-{}]({})\n".format(
                action, project_key, issue_key, url
            )

    for link in links:
        if "object" in link:
            richtext += "  * [{}]({})\n".format(
                link["object"]["title"] or link["object"]["url"], link["object"]["url"],
            )
...
class JiraClient:
    ...
    def get(self, uri_path, query_params=None):
        ...
        if response.status_code != 200:
            raise Exception(
                "Resource Unavailable: %s at %s" % (response.text, url), response
            )
    ...
    def raw_get(self, absolute_uri, query_params=None):
        ...
        if response.status_code == 401:
            raise Exception(
                "Unauthorized: %s at %s" % (response.text, absolute_uri), response
            )
        if response.status_code != 200:
            raise Exception(
                "Resource Unavailable: %s at %s" % (response.text, absolute_uri),
                response,
            )
...
class JiraImporterCommon:
    ...
    def resolve_user_bindings(self, options):
        ...
        options["users_bindings"] = {
            k: resolve_user(v)
            for k, v in options["users_bindings"].items()
            if v is not None
        }
    ...
    def list_users(self):
        ...
        for user in users:
            user_data = self._client.get("/user", {"key": user["key"]})
            result.append(
                {
                    "id": user_data["key"],
                    "full_name": user_data["displayName"],
                    "email": user_data["emailAddress"],
                    "avatar": user_data.get("avatarUrls", None)
                    and user_data["avatarUrls"].get("48x48", None),
                }
            )
        return result
        ...
        for custom_field in self._client.get("/field"):
            if custom_field["custom"]:
                if (
                    ...
                    custom_field["name"] == "Story Points"
                    and custom_field["schema"]["custom"]
    ...
    def _import_to_custom_fields(self, obj, issue, options):
        if isinstance(obj, Epic):
            ...
            raise NotImplementedError(
                "Not implemented custom attributes for this object ({})".format(obj)
            )
        ...
        for custom_field in self.custom_fields:
            ...
            if data:
                taiga_field = custom_att_manager.get(
    ...
    def _import_attachments(self, obj, issue, options):
        ...
        for attachment in issue["fields"]["attachment"]:
            try:
                ...
                att.attached_file.save(
                    attachment["filename"], ContentFile(data), save=True
                )
            except Exception:
    ...
    def _import_changelog(self, project, obj, issue, options):
        ...
        for history in sorted(
            issue["changelog"]["histories"], key=lambda h: h["created"]
        ):
    ...
    def _transform_history_data(self, project, obj, history, options):
        ...
        taiga_user = users_bindings.get(
            history.get("author", {}).get("key", None), None
        )
        if taiga_user:
        ...
        for history_item in history["items"]:
            if history_item["field"] == "Attachment":
                ...
                if history_item["from"] is not None:
                    try:
                        idx = obj.cummulative_attachments.index(
                            history_item["fromString"]
                        )
                        obj.cummulative_attachments.pop(idx)
                    except ValueError:
                        print(
                            "ERROR: Removing attachment that doesn't exist in the history ({})".format(
                                history_item["fromString"]
                            )
                        )
                ...
                result["change_old"]["description_html"] = mdrender(
                    obj.project, history_item["fromString"] or ""
                )
                result["change_new"]["description_html"] = mdrender(
                    obj.project, history_item["toString"] or ""
                )
                has_data = True
                ...
                if history_item["fromString"]:
                    try:
                        old_milestone = obj.project.milestones.get(
                            name=history_item["fromString"]
                        ).id
                ...
                if history_item["toString"]:
                    try:
                        new_milestone = obj.project.milestones.get(
                            name=history_item["toString"]
                        ).id
                    except Milestone.DoesNotExist:
                        new_milestone = -2
                ...
                if old_milestone == -1:
                    result["update_values"]["milestone"]["-1"] = history_item[
                        "fromString"
                    ]
                if new_milestone == -2:
                    result["update_values"]["milestone"]["-2"] = history_item[
                        "toString"
                    ]
                has_data = True
                ...
                if isinstance(obj, Task):
                    try:
                        old_status = obj.project.task_statuses.get(
                            name=history_item["fromString"]
                        ).id
                    except Exception:
                        old_status = -1
                    try:
                        new_status = obj.project.task_statuses.get(
                            name=history_item["toString"]
                        ).id
                    except Exception:
                        new_status = -2
                    ...
                    try:
                        old_status = obj.project.us_statuses.get(
                            name=history_item["fromString"]
                        ).id
                    except Exception:
                        old_status = -1
                    try:
                        new_status = obj.project.us_statuses.get(
                            name=history_item["toString"]
                        ).id
                    except Exception:
                        new_status = -2
                    ...
                    try:
                        old_status = obj.project.issue_statuses.get(
                            name=history_item["fromString"]
                        ).id
                    except Exception:
                        old_status = -1
                    ...
                    try:
                        old_status = obj.project.epic_statuses.get(
                            name=history_item["fromString"]
                        ).id
                    except Exception:
                        old_status = -1
                    try:
                        new_status = obj.project.epic_statuses.get(
                            name=history_item["toString"]
                        ).id
                    except Exception:
                        new_status = -2
                ...
                if history_item["fromString"]:
                    ...
                    (old_points, _) = Points.objects.get_or_create(
                        project=project,
                        value=estimation,
                        defaults={"name": str(estimation), "order": estimation,},
                    )
                ...
                result["change_old"]["points"] = {
                    project.roles.get(slug="main").id: old_points
                }
                result["change_new"]["points"] = {
                    project.roles.get(slug="main").id: new_points
                }
                has_data = True
                ...
                if isinstance(obj, Epic):
                    result["change_old"]["color"] = EPIC_COLORS.get(
                        history_item["fromString"], None
                    )
                    result["change_new"]["color"] = EPIC_COLORS.get(
                        history_item["toString"], None
                    )
                ...
                if isinstance(obj, Task):
                    field_obj = obj.project.taskcustomattributes.get(
                        name=custom_field["taiga_field_name"]
                    )
                elif isinstance(obj, UserStory):
                    field_obj = obj.project.userstorycustomattributes.get(
                        name=custom_field["taiga_field_name"]
                    )
                elif isinstance(obj, Issue):
                    field_obj = obj.project.issuecustomattributes.get(
                        name=custom_field["taiga_field_name"]
                    )
                elif isinstance(obj, Epic):
                    field_obj = obj.project.epiccustomattributes.get(
                        name=custom_field["taiga_field_name"]
                    )
    ...
    def _cleanup(self, project, options):
        for epic_custom_field in project.epiccustomattributes.all():
            if (
                project.epics.filter(
                    custom_attributes_values__attributes_values__has_key=str(
                        epic_custom_field.id
                    )
                ).count()
                == 0
            ):
