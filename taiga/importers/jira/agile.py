...
class JiraAgileImporter(JiraImporterCommon):
    def list_projects(self):
        return [
            ...
            for board in self._client.get_agile("/board")["values"]
        ]

    def list_issue_types(self, project_id):
        board_project = self._client.get_agile("/board/{}/project".format(project_id))[
            "values"
        ][0]
    ...
    def _import_project_data(self, project_id, options):
        ...
        project_template.default_options[
            "epic_status"
        ] = project_template.epic_statuses[0]["name"]
        project_template.default_options["us_status"] = project_template.us_statuses[0][
            "name"
        ]
        project_template.default_options[
            "task_status"
        ] = project_template.task_statuses[0]["name"]
        project_template.default_options[
            "issue_status"
        ] = project_template.issue_statuses[0]["name"]

        project_template.points = [{"value": None, "name": "?", "order": 0,}]
        ...
        import_service.create_memberships(
            options.get("users_bindings", {}), project, self._user, "main"
        )

        if project_template.slug == "scrum":
            for sprint in self._client.get_agile("/board/{}/sprint".format(project_id))[
                "values"
            ]:
    ...
    def _import_user_stories_data(self, project_id, project, options):
        ...
        project_conf = self._client.get_agile(
            "/board/{}/configuration".format(project_id)
        )
        ...
        while True:
            issues = self._client.get_agile(
                "/board/{}/issue".format(project_id),
                {"startAt": offset, "expand": "changelog",},
            )
            offset += issues["maxResults"]

            for issue in issues["issues"]:
                issue["fields"]["issuelinks"] += self._client.get(
                    "/issue/{}/remotelink".format(issue["key"])
                )
                ...
                if options.get("keep_external_reference", False):
                    external_reference = [
                        "jira",
                        self._client.get_issue_url(issue["key"]),
                    ]

                try:
                    milestone = project.milestones.get(
                        name=(issue["fields"].get("sprint", {}) or {}).get("name", "")
                    )
                ...
                us = UserStory.objects.create(
                    ...
                    status=project.us_statuses.get(
                        slug=slugify(issue["fields"]["status"]["name"])
                    ),
                ...
                try:
                    ...
                    RelatedUserStory.objects.create(user_story=us, epic=epic, order=1)
        ...
        while True:
            ...
            for issue in issues["issues"]:
                ...
                task = Task.objects.create(
                    ...
                    owner=owner,
                    assigned_to=assigned_to,
                    status=project.task_statuses.get(
                        slug=slugify(issue["fields"]["status"]["name"])
                    ),
                    subject=issue["fields"]["summary"],
                    description=issue["fields"]["description"] or "",
                ...
                for subtask in issue["fields"]["subtasks"]:
                    print(
                        "WARNING: Ignoring subtask {} because parent isn't a User Story".format(
                            subtask["key"]
                        )
                    )
    ...
    def _import_epics_data(self, project_id, project, options):
        ...
        while True:
            issues = self._client.get_agile(
                "/board/{}/epic".format(project_id), {"startAt": offset,}
            )
            offset += issues["maxResults"]
            ...
            for epic in issues["values"]:
                ...
                epic = Epic.objects.create(
                    ...
                    owner=owner,
                    assigned_to=assigned_to,
                    status=project.epic_statuses.get(
                        slug=slugify(issue["fields"]["status"]["name"])
                    ),
                    subject=issue["fields"]["summary"],
                    description=issue["fields"]["description"] or "",
                ...
                issue_with_changelog = self._client.get(
                    "/issue/{}".format(issue["key"]), {"expand": "changelog"}
                )
