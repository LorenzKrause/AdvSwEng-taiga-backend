...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    email = models.EmailField(
        _("email address"), max_length=255, blank=True, unique=True
    )
    ...
    colorize_tags = models.BooleanField(
        null=False, blank=True, default=False, verbose_name=_("colorize tags")
    )
    token = models.CharField(
        max_length=200, null=True, blank=True, default=None, verbose_name=_("token")
    )
    ...
    def is_watcher(self, obj):
        if self._cached_watched_ids is None:
            ...
            notify_policies = self.notify_policies.select_related("project").exclude(
    ...
    def get_notify_level(self, project):
        if self._cached_notify_levels is None:
            ...
            for notify_policy in self.notify_policies.select_related("project"):
                self._cached_notify_levels[
                    notify_policy.project.id
                ] = notify_policy.notify_level
    ...
    def cancel(self):
        with advisory_lock("delete-user"):
            ...
            self.username = slugify_uniquely(
                deleted_user_prefix, User, slugfield="username"
            )
...
class Role(models.Model):
    name = models.CharField(
        max_length=200, null=False, blank=False, verbose_name=_("name")
    )
