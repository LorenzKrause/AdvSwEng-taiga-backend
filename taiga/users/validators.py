...
class UserValidator(validators.ModelValidator):
    ...
    def validate_username(self, attrs, source):
        value = attrs[source]
        validator = core_validators.RegexValidator(
            re.compile("^[\w.-]+$"), _("invalid username"), _("invalid")
        )
        ...
        try:
            ...
            raise ValidationError(
                _(
                    "Required. 255 characters or fewer. Letters, "
        ...
        if (
            self.object
            and self.object.username != value
...
class UserAdminValidator(UserValidator):
    ...
    def validate_read_new_terms(self, attrs, source):
        value = attrs[source]
        if not value:
            raise ValidationError(_("Read new terms has to be true'"))
