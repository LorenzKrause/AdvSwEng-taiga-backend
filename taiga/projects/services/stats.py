...
def get_stats_for_project_issues(project):
    ...
    for issue in issues:
        ...
        _count_status_object(
            issue.priority, project_issues_stats["issues_per_priority"]
        )
        _count_status_object(
            issue.severity, project_issues_stats["issues_per_severity"]
        )
        _count_owned_object(issue.owner, project_issues_stats["issues_per_owner"])
        _count_owned_object(
            issue.assigned_to, project_issues_stats["issues_per_assigned_to"]
        )
    ...
    for severity in project_issues_stats["issues_per_severity"].values():
        ...
        del project_issues_stats["last_four_weeks_days"]["by_severity"][severity["id"]][
            "count"
        ]
        project_issues_stats["last_four_weeks_days"]["by_severity"][severity["id"]][
            "data"
        ] = []

    for priority in project_issues_stats["issues_per_priority"].values():
        project_issues_stats["last_four_weeks_days"]["by_priority"][
            priority["id"]
        ] = copy.copy(priority)
        del project_issues_stats["last_four_weeks_days"]["by_priority"][priority["id"]][
            "count"
        ]
        project_issues_stats["last_four_weeks_days"]["by_priority"][priority["id"]][
            "data"
        ] = []
    ...
    for x in range(27, -1, -1):
        ...
        open_this_day = filter(
            lambda x: x.created_date.replace(tzinfo=None) >= day, issues
        )
        open_this_day = filter(
            lambda x: x.created_date.replace(tzinfo=None) < next_day, open_this_day
        )
        ...
        closed_this_day = filter(
            lambda x: x.finished_date.replace(tzinfo=None) >= day, closed_this_day
        )
        closed_this_day = filter(
            lambda x: x.finished_date.replace(tzinfo=None) < next_day, closed_this_day
        )
        ...
        opened_this_day = filter(
            lambda x: x.created_date.replace(tzinfo=None) < next_day, issues
        )
        opened_this_day = list(
            filter(
                lambda x: x.finished_date is None
                or x.finished_date.replace(tzinfo=None) > day,
                opened_this_day,
            )
        )
        ...
        for severity in project_issues_stats["last_four_weeks_days"]["by_severity"]:
            ...
            project_issues_stats["last_four_weeks_days"]["by_severity"][severity][
                "data"
            ].append(by_severity)
        ...
        for priority in project_issues_stats["last_four_weeks_days"]["by_priority"]:
            ...
            project_issues_stats["last_four_weeks_days"]["by_priority"][priority][
                "data"
            ].append(by_priority)
...
def _get_milestones_stats_for_backlog(project, milestones):
    ...
    total_story_points = (
        ...
        if project.total_story_points not in [None, 0]
        else project._defined_points
    )

    total_milestones = (
        project.total_milestones
        if project.total_milestones not in [None, 0]
        else len(milestones)
    )
    ...
    for current_milestone_pos in range(0, max(milestones_count, total_milestones)):
        ...
        evolution = (
            total_story_points - current_evolution
    ...
    evolution = (
        total_story_points - current_evolution
        if current_evolution is not None and total_story_points
        else None
    )
...
def get_stats_for_project(project):
    ...
    def _find_milestone_for_userstory(user_story):
        for m in milestones.values():
            if (
                m.estimated_finish > user_story.created_date.date()
                and m.estimated_start <= user_story.created_date.date()
            ):
    ...
    for role_point in role_points:
        ...
        project._defined_points_for_role = project._defined_points_per_role.get(
            role_id, 0
        )
...
def _get_closed_bugs_per_member_stats(project):
    ...
    closed_bugs = (
        project.issues.filter(status__is_closed=True)
        .values("assigned_to")
...
def _get_iocaine_tasks_per_member_stats(project):
    ...
    iocaine_tasks = (
        project.tasks.filter(is_iocaine=True)
        .values("assigned_to")
        .annotate(count=Count("assigned_to"))
        .order_by()
    )
...
def _get_created_bugs_per_member_stats(project):
    # Created_bugs
    created_bugs = (
        project.issues.values("owner").annotate(count=Count("owner")).order_by()
    )
...
def _get_closed_tasks_per_member_stats(project):
    # Closed tasks
    closed_tasks = (
        project.tasks.filter(status__is_closed=True)
        .values("assigned_to")
        .annotate(count=Count("assigned_to"))
        .order_by()
    )
