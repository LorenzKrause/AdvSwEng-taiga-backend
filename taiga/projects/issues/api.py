...
class IssueViewSet(
    ...
    filter_backends = (
        filters.CanViewIssuesFilterBackend,
        filters.RoleFilter,
    ...
    filter_fields = ("milestone", "project", "project__slug", "status__is_closed")
    order_by_fields = (
        "type",
        "project",
        "status",
    ...
    def pre_conditions_on_save(self, obj):
        if obj.milestone and obj.milestone.project != obj.project:
            raise exc.PermissionDenied(
                _("You don't have permissions to set this sprint " "to this issue.")
            )

        if obj.status and obj.status.project != obj.project:
            raise exc.PermissionDenied(
                _("You don't have permissions to set this status " "to this issue.")
            )

        if obj.severity and obj.severity.project != obj.project:
            raise exc.PermissionDenied(
                _("You don't have permissions to set this severity " "to this issue.")
            )

        if obj.priority and obj.priority.project != obj.project:
            raise exc.PermissionDenied(
                _("You don't have permissions to set this priority " "to this issue.")
            )

        if obj.type and obj.type.project != obj.project:
            raise exc.PermissionDenied(
                _("You don't have permissions to set this type " "to this issue.")
            )
    ...
    def filters_data(self, request, *args, **kwargs):
        ...
        types_filter_backends = (
            f for f in filter_backends if f != filters.IssueTypesFilter
        )
        ...
        priorities_filter_backends = (
            f for f in filter_backends if f != filters.PrioritiesFilter
        )
        severities_filter_backends = (
            f for f in filter_backends if f != filters.SeveritiesFilter
        )
        ...
        querysets = {
            "types": self.filter_queryset(
                queryset, filter_backends=types_filter_backends
            ),
            ...
            "priorities": self.filter_queryset(
                queryset, filter_backends=priorities_filter_backends
            ),
            "severities": self.filter_queryset(
                queryset, filter_backends=severities_filter_backends
            ),
