...
def migrate_userstory_diff(obj: FrozenObj) -> FrozenObj:
    ...
    if "assigned_users" not in obj.snapshot.keys():
        ...
        snapshot["assigned_users"] = (
            [obj.snapshot["assigned_to"]] if obj.snapshot["assigned_to"] else []
        )
...
def make_diff(
    oldobj: FrozenObj, newobj: FrozenObj, excluded_keys: tuple = ()
) -> FrozenDiff:
    """
...
def get_last_snapshot_for_key(key: str) -> FrozenObj:
    ...
    qs = entry_model.objects.filter(key=key, is_snapshot=True).order_by("-created_at")
    ...
    entries = tuple(
        entry_model.objects.filter(key=key, is_snapshot=False)
...
def get_history_queryset_by_model_instance(
    obj: object, types=(HistoryType.change,), include_hidden=False
):
    """
