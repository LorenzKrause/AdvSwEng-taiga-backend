...
def connect_tasks_signals():
    ...
    signals.pre_save.connect(
        handlers.set_finished_date_when_edit_task,
...
def connect_tasks_close_or_open_us_and_milestone_signals():
    ...
    signals.pre_save.connect(
        handlers.cached_prev_task,
        sender=apps.get_model("tasks", "Task"),
        dispatch_uid="cached_prev_task",
    )
    # Open/Close US and Milestone
    signals.post_save.connect(
        handlers.try_to_close_or_open_us_and_milestone_when_create_or_edit_task,
        sender=apps.get_model("tasks", "Task"),
        dispatch_uid="try_to_close_or_open_us_and_milestone_when_create_or_edit_task",
    )
    signals.post_delete.connect(
        handlers.try_to_close_or_open_us_and_milestone_when_delete_task,
        sender=apps.get_model("tasks", "Task"),
...
def connect_tasks_custom_attributes_signals():
    ...
    signals.post_save.connect(
        custom_attributes_handlers.create_custom_attribute_value_when_create_task,
        sender=apps.get_model("tasks", "Task"),
...
def disconnect_tasks_signals():
    ...
    signals.pre_save.disconnect(
        sender=apps.get_model("tasks", "Task"), dispatch_uid="tags_normalization"
    )


def disconnect_tasks_close_or_open_us_and_milestone_signals():
    signals.pre_save.disconnect(
        sender=apps.get_model("tasks", "Task"), dispatch_uid="cached_prev_task"
    )
