...
def try_to_close_or_open_us_and_milestone_when_create_or_edit_task(
...
def _try_to_close_or_open_us_when_create_or_edit_task(instance):
    ...
    if (
        instance.prev
        and instance.prev.user_story_id
        and instance.prev.user_story_id != instance.user_story_id
    ):
...
def _try_to_close_milestone_when_delete_task(instance):
    ...
    with suppress(ObjectDoesNotExist):
        if instance.milestone_id and services.calculate_milestone_is_closed(
            instance.milestone
        ):
