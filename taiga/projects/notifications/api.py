...
class NotifyPolicyViewSet(ModelCrudViewSet):
    ...
    def _build_needed_notify_policies(self):
        ...
        for project in projects:
            services.create_notify_policy_if_not_exists(
                project, self.request.user, NotifyLevel.all
            )
    ...
    def get_queryset(self):
        ...
        return (
            models.NotifyPolicy.objects.filter(user=self.request.user)
            .filter(
...
class WebNotificationsViewSet(GenericViewSet):
    ...
    def check_permissions(self, request, obj=None):
        return (
            obj and request.user.is_authenticated() and request.user.pk == obj.user_id
        )
    ...
    def list(self, request):
        ...
        queryset = models.WebNotification.objects.filter(user=self.request.user)
        ...
        if page is not None:
            ...
            return response.Ok({"objects": serializer.data, "total": queryset.count()})
    ...
    def post(self, request):
        ...
        models.WebNotification.objects.filter(user=self.request.user).update(
            read=timezone.now()
        )
