...
class EpicAttachmentPermission(TaigaResourcePermission):
    ...
    create_perms = HasProjectPerm("modify_epic") | (
        CommentAttachmentPerm() & HasProjectPerm("comment_epic")
    )
...
class UserStoryAttachmentPermission(TaigaResourcePermission):
    ...
    create_perms = HasProjectPerm("modify_us") | (
        CommentAttachmentPerm() & HasProjectPerm("comment_us")
    )
...
class TaskAttachmentPermission(TaigaResourcePermission):
    ...
    create_perms = HasProjectPerm("modify_task") | (
        CommentAttachmentPerm() & HasProjectPerm("comment_task")
    )
...
class IssueAttachmentPermission(TaigaResourcePermission):
    ...
    create_perms = HasProjectPerm("modify_issue") | (
        CommentAttachmentPerm() & HasProjectPerm("comment_issue")
    )
...
class WikiAttachmentPermission(TaigaResourcePermission):
    ...
    create_perms = HasProjectPerm("modify_wiki_page") | (
        CommentAttachmentPerm() & HasProjectPerm("comment_wiki_page")
    )
...
class RawAttachmentPerm(PermissionComponent):
    def check_permissions(self, request, view, obj=None):
        ...
        if obj.content_type.app_label == "epics" and obj.content_type.model == "epic":
            return (
                EpicAttachmentPermission(request, view).check_permissions(
                    "retrieve", obj
                )
                or is_owner
            )
        elif (
            obj.content_type.app_label == "userstories"
            and obj.content_type.model == "userstory"
        ):
            return (
                UserStoryAttachmentPermission(request, view).check_permissions(
                    "retrieve", obj
                )
                or is_owner
            )
            ...
            return (
                TaskAttachmentPermission(request, view).check_permissions(
                    "retrieve", obj
                )
                or is_owner
            )
        elif (
            obj.content_type.app_label == "issues" and obj.content_type.model == "issue"
        ):
            return (
                IssueAttachmentPermission(request, view).check_permissions(
                    "retrieve", obj
                )
                or is_owner
            )
        elif (
            obj.content_type.app_label == "wiki"
            and obj.content_type.model == "wikipage"
        ):
            return (
                WikiAttachmentPermission(request, view).check_permissions(
                    "retrieve", obj
                )
                or is_owner
            )
