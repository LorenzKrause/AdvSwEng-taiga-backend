...
def calculate_milestone_is_closed(milestone):
    ...
    uss_check = (
        milestone.user_stories.all().count() > 0
        and all_tasks_closed
        and all_us_closed
        and all_issues_closed
    )
    issues_check = (
        milestone.issues.all().count() > 0
        and all_issues_closed
        and all_tasks_closed
        and all_us_closed
    )
    tasks_check = (
        milestone.tasks.all().count() > 0
        and all_tasks_closed
        and all_issues_closed
        and all_us_closed
    )
