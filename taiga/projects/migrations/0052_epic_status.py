...
def update_epic_status(apps, schema_editor):
    ...
    project_ids = Project.objects.filter(default_epic_status__isnull=True).values_list(
        "id", flat=True
    )
    if not project_ids:
        return
    ...
    for project_id in project_ids:
        values_sql.append(
            "('New', 'new', 1, false, '#999999', {project_id})".format(
                project_id=project_id
            )
        )
        values_sql.append(
            "('Ready', 'ready', 2, false, '#ff8a84', {project_id})".format(
                project_id=project_id
            )
        )
        values_sql.append(
            "('In progress', 'in-progress', 3, false, '#ff9900', {project_id})".format(
                project_id=project_id
            )
        )
        values_sql.append(
            "('Ready for test', 'ready-for-test', 4, false, '#fcc000', {project_id})".format(
                project_id=project_id
            )
        )
        values_sql.append(
            "('Done', 'done', 5, true, '#669900', {project_id})".format(
                project_id=project_id
            )
        )
