...
def create_user_project_settings(project, user, homepage=Section.timeline):
    ...
    model_cls = apps.get_model("settings", "UserProjectSettings")
    try:
        return model_cls.objects.create(project=project, user=user, homepage=homepage)
...
def create_user_project_settings_if_not_exists(
    project, user, homepage=Section.timeline
):
    """
    Given a project and user, create notification policy for it.
    """
    ...
    try:
        result = model_cls.objects.get_or_create(
            project=project, user=user, defaults={"homepage": homepage}
        )
        return result[0]
    except IntegrityError as e:
        raise exc.IntegrityError(
