...
ATTACHMENT_SAMPLE_DATA = ["taiga/projects/management/commands/sample_data", [".txt",]]
...
NUM_APPLICATIONS_TOKENS = getattr(
    settings, "SAMPLE_DATA_NUM_APPLICATIONS_TOKENS", (1, 3)
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for x in projects_range:
            project = self.create_project(
                ...
                if x in (blocked_projects_range)
                else None,
            )
            ...
            if x not in empty_projects_range:
                ...
                for y in range(self.sd.int(*NUM_MILESTONES)):
                    ...
                    for z in range(self.sd.int(*NUM_USS)):
                        us = self.create_us(
                            project, milestone, computable_project_roles
                        )
    ...
    def create_wiki_link(self, project, title=None):
        wiki_link = WikiLink.objects.create(
            project=project, title=title or self.sd.words(1, 3)
        )
        return wiki_link
    ...
    def create_wiki_page(self, project, slug):
        wiki_page = WikiPage.objects.create(
            ...
            owner=self.sd.db_object_from_queryset(
                project.memberships.filter(user__isnull=False)
            ).user,
        )
        ...
        wiki_page.save()
        take_snapshot(wiki_page, comment=self.sd.paragraph(), user=wiki_page.owner)
    ...
    def create_epic(self, project):
        ...
        if self.sd.choice([True, True, False, True, True]):
            ...
            for idx, us in enumerate(list(user_stories)):
                RelatedUserStory.objects.create(epic=epic, user_story=us, order=idx + 1)
    ...
    def create_project(self, counter, is_private=None, blocked_code=None):
        ...
        anon_permissions = (
            not is_private and list(map(lambda perm: perm[0], ANON_PERMISSIONS)) or []
        )
        public_permissions = (
            not is_private and list(map(lambda perm: perm[0], ANON_PERMISSIONS)) or []
        )
