...
class Application(models.Model):
    id = models.CharField(
        primary_key=True, max_length=255, unique=True, default=_generate_uuid
    )
...
class ApplicationToken(models.Model):
    ...
    @property
    def next_url(self):
        return "{url}?auth_code={auth_code}".format(
            url=self.application.next_url, auth_code=self.auth_code
        )
